...
export function adjustPanelHeight() {
  ...
  const detailEl =
    sheet.querySelector('[data-view="summary"]:not([hidden])') ||
    sheet.querySelector('[data-view="result"]:not([hidden])');
  const offset = 40;
  // если mainEl нет — используем высоту всего sheet как резерв
  const maxHeight = mainEl ? mainEl.scrollHeight + offset : sheet.scrollHeight;

  // если detailEl нет — сворачиваем вплоть до offset
  const minHeight = detailEl ? detailEl.scrollHeight + offset : offset + 20;
